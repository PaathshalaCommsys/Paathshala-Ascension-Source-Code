...
public class DataQuery {
                ...
                try {
                    ...
                    if (set != null && set.set != null) {
                        AndroidUtilities.runOnUIThread(() -> {
                            ...
                            NotificationCenter.getInstance(currentAccount).postNotificationName(NotificationCenter.groupStickersDidLoad, set.set.id);
                        });
                    }
                } catch (Throwable e) {
                    FileLog.e(e);
            ...
            try {
                ...
                if (lower_part == 0) {
                    for (int a = 0; a < counts.length; a++) {
                        if (counts[a] == -1) {
                            cursor = MessagesStorage.getInstance(currentAccount).getDatabase().queryFinalized(String.format(Locale.US, "SELECT COUNT(mid) FROM media_v2 WHERE uid = %d AND type = %d LIMIT 1", uid, a));
                            if (cursor.next()) {
    ...
    private void processLoadedMediaCount(final int count, final long uid, final int type, final int classGuid, final boolean fromCache, int old) {
        AndroidUtilities.runOnUIThread(() -> {
            ...
            if (!reload) {
                ...
                NotificationCenter.getInstance(currentAccount).postNotificationName(NotificationCenter.mediaCountDidLoad, uid, (fromCache && count == -1 ? 0 : count), fromCache, type);
            }
        });
    }
    ...
    public void loadMusic(final long uid, final long max_id) {
        MessagesStorage.getInstance(currentAccount).getStorageQueue().postRunnable(() -> {
            ...
            AndroidUtilities.runOnUIThread(() -> NotificationCenter.getInstance(currentAccount).postNotificationName(NotificationCenter.musicDidLoad, uid, arrayList));
        });
    }
    //---------------- MEDIA END ----------------
    ...
    public MessageObject loadPinnedMessage(final long dialogId, final int channelId, final int mid, boolean useQueue) {
        if (useQueue) {
            MessagesStorage.getInstance(currentAccount).getStorageQueue().postRunnable(() -> loadPinnedMessageInternal(dialogId, channelId, mid, false));
        } else {
    ...
    public void loadBotKeyboard(final long did) {
        ...
        if (keyboard != null) {
            NotificationCenter.getInstance(currentAccount).postNotificationName(NotificationCenter.botKeyboardDidLoad, keyboard, did);
            return;
        }
        MessagesStorage.getInstance(currentAccount).getStorageQueue().postRunnable(() -> {
            try {
                ...
                if (botKeyboard != null) {
                    ...
                    AndroidUtilities.runOnUIThread(() -> NotificationCenter.getInstance(currentAccount).postNotificationName(NotificationCenter.botKeyboardDidLoad, botKeyboardFinal, did));
                }
            } catch (Exception e) {
                FileLog.e(e);
            }
